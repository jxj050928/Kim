...
{
    public class JMPieMainChart : Panel
    {
        #region 私有变量
        ...
        private void InitializeComponent()
        {
            ...
            this.jmButtonTriangle1.Location = new System.Drawing.Point(326, -10);
            ...
            this.jmButtonTriangle2.Location = new System.Drawing.Point(326, 225);
        ...
        {
            clickshu++;
            if (clickshu > m_values.Length - 10)
            {
                clickshu--;
                jmButtonTriangle2.Visible = false;
            }
        ...
        [Description("是否显示右侧详细列表")]
        public bool Zbool
        {
            get { return m_bool; }
            set { m_bool = value; Invalidate(); }
        }
            ...
            if (getIsInner(e.Location))
            {
                ...
                for (int i = 0; i < allRadius.Length; i++)
                {
                    if (angle <= allRadius[i])
                    {
                        ...
                        if (m_text == null)
                        {
                            bfb = sumValue == 0 ? "0.00%" : (m_values[i] / sumValue).ToString("p");
                            tiptext = m_values[i].ToString() + " (" + bfb + ")";
                        }
                        ...
                        {
                            bfb = sumValue == 0 ? "0.00%" : (m_values[i] / sumValue).ToString("p");
                            tiptext = m_text[i] + " " + m_values[i].ToString() + " (" + bfb + ")";
                        }
        ...
        {
            double dA = Math.Pow(MoveP.X - PieOrigin.X - Padding.Left, 2) / Math.Pow(PieOrigin.X + Padding.Left, 2);
            double dB = Math.Pow(MoveP.Y - PieOrigin.Y - Padding.Top, 2) / Math.Pow(PieOrigin.Y + Padding.Top, 2);
            //double dA = Math.Pow(MoveP.X + 15, 2) / Math.Pow(PieWidth, 2);
            //double dB = Math.Pow(MoveP.Y + PieHeight / 2, 2) / Math.Pow(PieHeight, 2);
            if (dA + dB <= 1)
            {
                if (MoveP.X >= PieOrigin.X - PieWidth / 2 + Padding.Left && MoveP.X <= PieOrigin.X + PieWidth / 2 + Padding.Left)
                {
                    if (MoveP.Y >= PieOrigin.Y - PieHeight / 2 + Padding.Top && MoveP.Y <= PieOrigin.Y + PieHeight / 2 + Padding.Top)
                    {
                        return true;
                    }
                }
        ...
        {
            cen = new Point(cen.X + Padding.Left, cen.Y + Padding.Top);
            second = new Point(second.X + Padding.Left, second.Y + Padding.Top);
