...
{
    public class JMPiePaymentChart : Panel
    {
        #region 私有变量
        ...
        private Point z_PieLocacion;

        public Point Z_PieLocacion
        {
            get { return z_PieLocacion; }
            set { z_PieLocacion = value; Invalidate(); }
        }
        public int Z_Scroll
        {
            get { return scroll; }
            ...
            : base()
        {
            ...
            m_pieWidth = 254;
            m_pieHeight = 254;
            m_pieOrigin = new Point(m_pieWidth / 2 + z_PieLocacion.X, m_pieHeight / 2 + z_PieLocacion.X);
            m_height3D = 15;
            this.Size = new Size(510, 242);
            z_PieLocacion = new Point(15, 15);
            InitializeComponent();
        }
        #endregion
        ...
        {
            CanMoveScroll = 0;
            Rectangle rec = new Rectangle(z_PieLocacion.X, z_PieLocacion.Y, PieWidth, PieHeight);//
            ...
            else
            {
                ...
                for (int i = 0; i < m_values.Length; i++)
                {
                    ...
                    {
                        //画明细
                        Rectangle rectmx = new Rectangle(Width / 2 + 20, (i + 1) * 20 + 3 + scroll, 11, 11);
                        ...
                        {
                            string st = m_text[i].Length > 10 ? m_text[i].Substring(0, 10).ToString() + "..." : m_text[i];
                            string money = m_values[i].ToString().Length > 12 ? m_values[i].ToString().Substring(0, 9) + "..." : m_values[i].ToString();
                            //百分比
                            strbfb = sumValue == 0 ? "0.00%" : (m_values[i] / sumValue).ToString("p");
